...
def _is_placeholder_session_title(current_title: str) -> bool:
    """True for empty/default/source-label titles that should be replaced."""
    current = re.sub(r'\s+', ' ', str(current_title or '')).strip()
    if not current:
        return True
    low = current.lower()
    if low in _PLACEHOLDER_SESSION_TITLES:
        return True
    # "{Source} Session" fallback from state.db projection (Webui Session, Api_Server Session, …)
    if low.endswith(' session') and len(low) <= 40:
        return True
    return False
...
def _is_provisional_title(current_title: str, messages) -> bool:
    ...
    if _is_placeholder_session_title(current_title):
        return True
    ...
    try:
        ...
        try:
            ...
            with _agent_lock:
                ...
                # Only auto-generate title when still default/placeholder; preserves user renames
                if _is_placeholder_session_title(s.title):
                    s.title = title_from(s.messages, s.title or 'Untitled')
                _looks_default = _is_placeholder_session_title(s.title)
