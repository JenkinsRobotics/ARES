...
def _load_cli_sessions_uncached(
    ...
    *,
    ...
    for row in read_importable_agent_session_rows(
        ...
        _display_title = (_title.strip() if isinstance(_title, str) and _title.strip() else '') or 'New chat'
        cli_sessions.append({
            'session_id': sid,
        ...
        try:
            for row in read_importable_agent_session_rows(
                ...
                if _sidecar_meta.get('title'):
                    _title = _sidecar_meta['title']
                _archived = bool(_sidecar_meta.get('archived'))
                _display_title = (_title.strip() if isinstance(_title, str) and _title.strip() else '') or 'Cron job'
                cli_sessions.append({
                    'session_id': sid,
                    'title': _display_title,
                    'workspace': _cli_workspace(),
        ...
        try:
            for row in read_importable_agent_session_rows(
                ...
                if _sidecar_meta.get('title'):
                    _title = _sidecar_meta['title']
                _archived = bool(_sidecar_meta.get('archived'))
                _display_title = (_title.strip() if isinstance(_title, str) and _title.strip() else '') or 'Webhook run'
                cli_sessions.append({
                    'session_id': sid,
                    'title': _display_title,
