...
class DispatchService:
    ...
    def __init__(
        self,
        backend_registry: type[BackendRegistry] | Any = BackendRegistry,
    ...
    def dispatch_turn(
        ...
        adapter = None
        worker = None
        if self.adapter_registry is None and self.registry is BackendRegistry:
            from fastapi_app.adapters import AdapterRegistry

            self.adapter_registry = AdapterRegistry()
        if self.adapter_registry is not None:
            try:
                adapter = self.adapter_registry.execution_adapter(assigned_worker)
            except Exception as exc:
                logger.error("Adapter resolution failed for %s: %s", assigned_worker, exc)
                return {
                    "plan_id": plan_id,
            ...
            worker = (self.registry.get_available() if self.registry else {}).get(assigned_worker)

        start_time = time.time()
        execution_payload: dict[str, Any] = {}

        if adapter is None and worker is None:
            return {
                "plan_id": plan_id,
                "status": "execution_failed",
                "assigned_worker": assigned_worker,
        ...
        if adapter is not None:
            try:
                ...
                execution_payload = dict(turn_res)
                raw_output = turn_res.get("text") or turn_res.get("response") or str(turn_res)
            except Exception as exc:
                logger.error("Adapter %s execution failed: %s", assigned_worker, exc)
                return {
                    "plan_id": plan_id,
                    "status": "execution_failed",
                    "assigned_worker": assigned_worker,
                    "error": "Selected runtime failed to execute the turn.",
                }
        ...
        model_name = str(
            execution_payload.get("model")
            or (getattr(worker, "model_name", "") if worker else "")
            or model
            or "unknown"
        )
        input_tokens = int(execution_payload.get("input_tokens") or 0)
        output_tokens = int(execution_payload.get("output_tokens") or 0)
