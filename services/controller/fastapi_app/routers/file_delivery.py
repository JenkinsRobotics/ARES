...
# ── Delegation log streaming ────────────────────────────────────────────────

@router.get("/api/delegation/logs")
def get_delegation_logs(
    identity: Annotated[RequestIdentity, Depends(require_identity)],
):
    """List active delegation tasks and their log paths.
    
    Polls ~/.hermes/cache/delegation/live/ for running/completed agents.
    Returns delegation ID, task count, log file paths, and goals.
    """
    from pathlib import Path
    import os
    ...
    # Scan for delegation directories
    for deleg_dir in sorted(live_dir.iterdir()):
        if not deleg_dir.is_dir():
            continue
        
        deleg_id = deleg_dir.name
        log_files = sorted(deleg_dir.glob("task-*.log"))
        
        if not log_files:
            continue
        
        # Try to read goals from the delegation metadata if it exists
        goals = []
        meta_file = deleg_dir / "meta.json"
        if meta_file.exists():
            try:
                import json
                meta = json.loads(meta_file.read_text())
                goals = meta.get("goals", [""] * len(log_files))
            except:
                goals = [""] * len(log_files)
        else:
            goals = [""] * len(log_files)
        ...
        delegations.append({
            "id": deleg_id,
            "taskCount": len(log_files),
            "logPaths": [str(log) for log in log_files],
            "goals": goals[:len(log_files)],
            "status": "running",  # Could check process status if needed
        })
...
def read_delegation_log(
    identity: Annotated[RequestIdentity, Depends(require_identity)],
    path: str = Query(min_length=1, max_length=4096),
):
    """Read a specific delegation log file.
    
    Streams the content of a live log file for real-time viewing.
    """
    from pathlib import Path
    
    log_path = Path(path)
    
    # Security: only allow reading from the delegation live directory
    live_dir = Path.home() / ".hermes" / "cache" / "delegation" / "live"
    try:
        log_path.resolve().relative_to(live_dir.resolve())
    except ValueError:
        raise CoreApiError(403, "Access denied: log path must be in delegation live directory")
    
    if not log_path.exists():
        raise CoreApiError(404, "Log file not found")
    
    try:
        content = log_path.read_text(errors="replace")
        return {"content": content}
    except Exception as exc:
        raise CoreApiError(500, f"Failed to read log: {exc}")
