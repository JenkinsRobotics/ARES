...
# Tailscale + carrier-grade NAT shared space (RFC 6598). Phone/mesh clients
# arrive as 100.64/10. Python's ipaddress.is_private is False for this range on
# some versions, which made auth-disabled ARES treat the phone as "public" and
# trip local-only gates (onboarding/terminal).
...
def ip_is_loopback_or_private(raw: str) -> tuple[bool, bool]:
    ...
    if mapped is not None and mapped in _TAILSCALE_CGNAT:
        return True, True
