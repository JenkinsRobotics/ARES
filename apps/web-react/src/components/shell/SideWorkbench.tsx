...
import { AgentsTab } from "@/components/shell/AgentsTab";
...
type WorkbenchTab = "files" | "artifacts" | "agents";
...
export function SideWorkbench({ onCollapse }: { onCollapse?: () => void }) {
  ...
  return (
    <section style={{ height: "100%", display: "flex", flexDirection: "column", background: H.bg, color: H.text, borderLeft: `1px solid ${H.border}` }}>
      ...
      <div style={{ padding: "0.5rem 0.75rem", borderBottom: `1px solid ${H.border}`, display: "flex", alignItems: "center", gap: "0.375rem", flexShrink: 0 }}>
        ...
        <button
          type="button"
          onClick={() => chooseTab("agents")}
          style={{
            padding: "0.25rem 0.875rem",
            borderRadius: "1.25rem",
            border: `1px solid ${tab === "agents" ? H.border2 : "transparent"}`,
            background: tab === "agents" ? H.surface : "transparent",
            color: tab === "agents" ? H.strong : H.muted,
            fontSize: "0.75rem",
            fontWeight: 500,
            cursor: "pointer",
            display: "flex",
