...
the "/goal" command and its goal manager). A Mission takes one
...
call (api/llm_client.py), and dispatches each sub-task either:
  - to the JaegerAI backend, via an ephemeral sub-session and
...
_DECOMPOSE_SYSTEM_PROMPT = (
    ...
    '"backend": "jaeger"|"anthropic"|"openai"}. Use "coding" + '
    '"jaeger" for work needing a terminal/file/tool-using agent. Use '
...
def _decompose(prompt: str) -> List[Dict[str, Any]]:
    ...
    for item in parsed:
        ...
        if backend not in ("jaeger", "anthropic", "openai"):
            backend = "jaeger" if kind == "coding" else "anthropic"
        subtasks.append({
            "id": _new_id(),
...
def _run_agentic_subtask(mission: Dict[str, Any], subtask: Dict[str, Any], backend: str) -> str:
    """Dispatch a sub-task through the real JaegerAI agent loop.
