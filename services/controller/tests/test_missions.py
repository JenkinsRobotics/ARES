...
Hermes/JaegerAI agent loop (ephemeral sub-session + start_session_turn) or a
...
real Hermes/JaegerAI install, or a live server.
"""
import time
...
def test_run_agentic_subtask_extracts_last_assistant_message(monkeypatch):
    ...
    result = missions._run_agentic_subtask({"profile": None}, subtask, "jaeger")
    assert result == "done, here's the PR"
    assert fake_session.ares_backend == "jaeger"
...
def test_run_agentic_subtask_raises_on_start_failure(monkeypatch):
    ...
    with pytest.raises(RuntimeError):
        missions._run_agentic_subtask({"profile": None}, {"description": "x"}, "jaeger")
