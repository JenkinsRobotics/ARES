...
def configured_root_override() -> tuple[str, str] | None:
    ...
    Explicit overrides are fail-closed: a value pointing at a deleted or legacy
    checkout suppresses discovery entirely rather than silently falling back to
    a different install. That is the right safety behavior, but on its own it is
    indistinguishable from "nothing installed" — so status surfaces need to know
    an override was responsible in order to say which variable to fix.
    """
    for name in (
