        ...
        try:
            ...
            if session is not None:
                ...
                if user_message.strip() and not (
                    ...
                    == " ".join((original_message or user_message).split())
                ):
                    session.messages.append({
                        "role": "user",
                        "content": original_message or user_message,
...
def run_hermes_streaming(
    ...
    *,
    ...
    # `msg_text` is the execution prompt and may be context-wrapped; these
    # aliases carry the text the user actually typed. Only that may be
    # persisted as a role=user message.
    _typed_user_text = next(
        (v for v in (user_text, original_message) if v is not None), None
    )
    ...
    if not cli:
        ...
        _finish_hermes_stream(stream_id, session_id, q, run_journal, cancel_event, accumulated_text="", user_message=msg_text, original_message=_typed_user_text)
        return

    from api.providers.hermes.backend import resolve_hermes_route

    effective_model, effective_provider = resolve_hermes_route(model, model_provider)
    ...
    try:
        ...
        _finish_hermes_stream(
            ...
            original_message=_typed_user_text,
        )
