...
def _gateway_base_url(config_data=None, environ: dict[str, str] | None = None, backend_id: str | None = None) -> str:
    source = os.environ if environ is None else environ
    cfg = config_data if isinstance(config_data, dict) else {}
...
def _run_gateway_chat_streaming(
    ...
    *,
    ...
    """
    # Only the text the user actually typed may become a visible role=user row.
    _visible_user_text = next(
    ...
    try:
        ...
        try:
            ...
            prefill_messages = []
        base_url = _gateway_base_url(cfg, backend_id=backend_id)
        ...
        with _get_session_agent_lock(session_id):
            ...
            user_msg = {"role": "user", "content": _visible_user_text, "timestamp": now}
            ...
            try:
                ...
                if display:
                    ...
                    if isinstance(latest, dict) and latest.get("role") == "user":
                        ...
                        msg_norm = " ".join(_visible_user_text.split())
