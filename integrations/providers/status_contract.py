...
# Default next action per blocking state, used when a provider has not supplied
# a more specific one. Generic, but never empty: a user facing a stopped chat
# should always be told what kind of thing to go do.
_DEFAULT_FIX_BY_STATE = {
    ProviderStatusState.NOT_INSTALLED: (
        "Install this runtime, or pick a different provider on the Connections page."
    ),
    ProviderStatusState.NOT_CONFIGURED: (
        "Add the missing credential or endpoint for this runtime in Settings, "
        "or pick a different provider on the Connections page."
    ),
    ProviderStatusState.OFFLINE: (
        "Start this runtime, then retry. If it is already running, check that "
        "its address and port match what ARES is configured to use."
    ),
    ProviderStatusState.NEEDS_ATTENTION: (
        "Finish this runtime's setup, or pick a different provider on the "
        "Connections page."
    ),
}


def remediation(status: ProviderStatus) -> dict[str, Any]:
    """Return ``{reason, fix}`` (plus any hints) for a provider status.

    A failure the user cannot act on is a dead end, so this guarantees both
    keys are present on every non-connected status. Providers that know the
    real cause should put ``reason``/``fix`` in their ``details`` — this only
    supplies a floor for the ones that have not, rather than letting a bare
    state code reach the UI.
    """
    details = dict(status.details or {})
    payload: dict[str, Any] = {
        "reason": str(details.get("reason") or status.message or "").strip(),
        "fix": str(
            details.get("fix") or _DEFAULT_FIX_BY_STATE.get(status.state, "") or ""
        ).strip(),
    }
    for hint in ("env_hint", "configured_by", "configured_root", "mode", "root"):
        if details.get(hint):
