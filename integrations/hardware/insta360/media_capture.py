...
from integrations.hardware.base import AudioSample, CameraFrame
...
class AVFoundationCapture:
    ...
    def capture_frame(
        ...
        if output_path is None:
            output_path = Path("/tmp") / f"ares_frame_{int(time.time()*1000)}.jpg"
    ...
    def record_audio(
        ...
        if output_path is None:
            output_path = Path("/tmp") / f"ares_audio_{int(time.time()*1000)}.wav"
        output_path = Path(output_path).resolve()
        output_path.parent.mkdir(parents=True, exist_ok=True)
