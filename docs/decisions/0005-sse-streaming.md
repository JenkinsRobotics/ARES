...
> **Superseded by [ADR-0007](./0007-chat-streaming-is-websocket.md).** This
> record was inaccurate when written: on the date below, `chat-stream.ts`
> already used WebSocket. It describes the decision ARES *inherited from
> upstream Hermes WebUI*, not what ARES implemented. Retained unedited as
> history; do not cite it as current architecture.

**Status:** Superseded (was: Accepted (inherited))
