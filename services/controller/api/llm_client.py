...
through the existing JaegerAI backend (webui/api/backends/), which carries
...
def call_anthropic(prompt: str, *, system: str | None = None, model: str | None = None, max_tokens: int = 4096) -> str:
    ...
    if not key:
        raise LLMProviderUnavailable(
            "No Anthropic API key configured. Set ANTHROPIC_API_KEY in ~/.ares/.env or the environment."
        )
...
def call_openai(prompt: str, *, system: str | None = None, model: str | None = None, max_tokens: int = 4096) -> str:
    ...
    if not key:
        raise LLMProviderUnavailable(
            "No OpenAI API key configured. Set OPENAI_API_KEY in ~/.ares/.env or the environment."
        )
