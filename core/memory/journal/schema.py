...
def _repair_messages_fts(db: sqlite3.Connection) -> bool:
    """Rebuild ``messages_fts`` when it is not actually an FTS5 table.

    Older journals were created on a build where the FTS5 statement did not
    take, leaving a plain ``CREATE TABLE messages_fts (content TEXT)`` behind.
    ``CREATE VIRTUAL TABLE IF NOT EXISTS`` then quietly does nothing because the
    name is taken, so the schema looks applied while every search fails with
    "no such column: messages_fts" — MATCH against a non-FTS table parses as a
    column reference. Detect that shape and rebuild it from ``messages``.
    """
    row = db.execute(
...
def search(query: str, source: Optional[str] = None, limit: int = 20) -> list[dict]:
    ...
    matching_conv_ids = db.execute(
        # The MATCH operand must be the FTS table itself, so the full-text
        # filter goes in a subquery rather than the join — an aliased table
        # resolves as a column name here and raises "no such column".
