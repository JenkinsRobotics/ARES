"""Profile-scoped CalDAV configuration, synchronization, and event writes.

ARES owns calendar metadata and its bounded offline cache. Credentials live in
the operating-system credential vault; this module only persists a secret key
reference. The HTTP client intentionally implements the small CalDAV subset we
need instead of coupling work management to a second scheduling framework.
"""
...
from api.secret_vault import get_secret, set_secret
...
def _state_file(profile: str | None, name: str) -> Path:
    ...
    return Path(STATE_DIR) / f"caldav.{safe_profile}.{name}.json"
...
def configure(
    ...
    if password:
        set_secret(profile, SECRET_KEY, password)
...
def _credentials(profile: str | None) -> tuple[dict[str, Any], str]:
    ...
    return config, get_secret(profile, SECRET_KEY)
...
def _request(
    ...
    *,
    ...
    request.add_header("User-Agent", "ARES-CalDAV/1")
    for key, value in (headers or {}).items():
        request.add_header(key, value)
    try:
        with urlopen(request, timeout=DEFAULT_TIMEOUT_SECONDS) as response:  # noqa: S310 - validated configured URL
...
def put_event(
    ...
    *,
    ...
    config, password = _credentials(profile)
    event_uid = uid or f"{uuid.uuid4().hex}@ares"
    ...
    ics = "\r\n".join(
        [
            "BEGIN:VCALENDAR",
            "VERSION:2.0",
            "PRODID:-//ARES//Work Management//EN",
            "BEGIN:VEVENT",
            f"UID:{event_uid}",
            f"DTSTAMP:{now}",
            f"DTSTART:{start}",
