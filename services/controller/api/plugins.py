...
def get_plugin_metadata() -> list[dict]:
    ...
    for name, manifest in sorted(PLUGIN_MANIFESTS.items()):
        ...
        plugins.append({
            ...
            "enabled": bool(plugin_settings.get(name, True)),
            "hooks": manifest.get("hooks", ["ui_tab"]),
        })

    # Built-in agent subsystems and plugins
    builtin_plugins = [
        {
            "name": "Memory Knowledge Graph",
            "key": "memory",
            "version": "2.4.0",
            "description": "Episodic memory store, entity relationship graph, and semantic fact retrieval engine.",
            "tab": {"path": "/memory", "label": "Memory"},
            "enabled": True,
            "hooks": ["pre_turn", "post_turn", "context_engine", "tool_call"],
        },
        {
            "name": "Interactive Kanban Board",
            "key": "kanban",
            "version": "1.8.2",
            "description": "Visual agile task management, milestone tracking, and autonomous session board updates.",
            "tab": {"path": "/kanban", "label": "Kanban"},
            "enabled": True,
            "hooks": ["ui_tab", "tool_call"],
        },
        {
            "name": "Voice & Speech Engine",
            "key": "voice",
            "version": "2.1.0",
            "description": "Realtime Whisper STT speech-to-text and Kokoro/ElevenLabs neural TTS with acoustic cancellation.",
            "tab": {"path": "/voice", "label": "Voice"},
            "enabled": True,
            "hooks": ["stt_cascade", "tts_synthesis", "audio_player"],
        },
        {
            "name": "Web Scraping & Search",
            "key": "web",
            "version": "1.5.0",
            "description": "Autonomous browser search, dynamic DOM extraction, and URL markdown ingestion.",
            "tab": {"path": "/web", "label": "Web"},
            "enabled": True,
            "hooks": ["tool_call", "search_provider"],
        },
        {
            "name": "Cron Scheduler & Triggers",
            "key": "cron_providers",
            "version": "1.3.0",
            "description": "Scheduled jobs, recurring cron tasks, and background autonomous worker wakeup.",
            "tab": {"path": "/cron", "label": "Cron"},
            "enabled": True,
            "hooks": ["scheduler", "task_delivery"],
        },
        {
            "name": "Security & Sandbox Guardrails",
            "key": "security-guidance",
            "version": "2.0.1",
            "description": "Deterministic command verification, dangerous action sandboxing, and workspace path containment.",
            "tab": {"path": "/security", "label": "Security"},
            "enabled": True,
            "hooks": ["command_filter", "permission_gate"],
        },
        {
            "name": "Observability & Performance Tracing",
            "key": "observability",
            "version": "1.4.2",
            "description": "Turn latency breakdown, token telemetry, step-by-step LLM trace logging, and cost analytics.",
            "tab": {"path": "/observability", "label": "Observability"},
            "enabled": True,
            "hooks": ["post_turn", "telemetry_logger"],
        },
    ]

    for p in builtin_plugins:
        if p["key"] not in seen_keys:
            p["enabled"] = bool(plugin_settings.get(p["key"], p.get("enabled", True)))
            plugins.append(p)
