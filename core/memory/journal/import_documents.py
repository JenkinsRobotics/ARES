...
def scan_documents(
    ...
    Args:
        ...
        include_globs: Filename patterns (``*.md``) to accept. If None, every
            readable extension in ``SCAN_EXTENSIONS`` is taken. Patterns are
            still intersected with that set — the parser only understands those
            formats, so a pattern cannot widen it, only narrow it.
    ...
    scan_paths = [Path(p).expanduser() for p in scan_paths]
    patterns = [p for p in (include_globs or []) if str(p).strip()] or None
