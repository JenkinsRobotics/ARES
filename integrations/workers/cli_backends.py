...
class OllamaLocalBackend(AgenticBackend):
    ...
    def _resolve_default_model(self) -> str | None:
        """The first actually-installed model, or None if Ollama has none.

        Never guess a plausible-sounding model name (the previous hardcoded
        "llama3.2" default silently 404'd on any machine that hadn't pulled
        that exact tag — confirmed live: this machine has gemini-3-flash-
        preview:latest / qwen3.6:35b-mlx / gemma4:31b-mlx installed, none of
        them "llama3.2"). CLAUDE.md: no silent default worker.
        """
    ...
    def run_turn(self, message: str, session_id: str, **kwargs) -> dict[str, Any]:
        ...
        config = kwargs.get("config") or kwargs.get("adapter_config") or {}
        model = _cfg_str(config, "model") or kwargs.get("model") or self._resolve_default_model()
        if not model:
            return {"text": "", "error": "No Ollama models installed (ollama pull <model>).", "tool_activity": []}
        cancel_event = kwargs.get("cancel_event")
        publish = kwargs.get("publish")
        ...
        try:
            ...
            if publish:
                ...
                # Parity with the streaming branch above: without this, a
                # 404 (unknown model) or other Ollama error response has no
                # "message" key, so msg.get("content", "") below silently
                # returned "" with error=None — confirmed live (curl to
                # /api/chat with an uninstalled model returns HTTP 404 +
                # {"error": "model '...' not found"}, previously swallowed).
                r.raise_for_status()
                data = r.json()
