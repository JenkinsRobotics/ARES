...
export default function SkillsPage() {
  ...
  return (
    <div className="page-stack">
      <PageHeader title="Skills" description={`${skills.length} available across ${categories} categories. Enable, inspect, or create reusable capabilities.`} action={<div className="flex gap-2"><Button size="sm" variant="outline" onClick={() => void load()} disabled={loading}><RefreshCw className={loading ? "animate-spin" : ""} />Refresh</Button><Button size="sm" asChild><Link to="/control?tab=skills&view=studio"><Plus />New skill</Link></Button></div>} />
      ...
      {loading ? <div className="grid place-items-center py-16 text-sm text-muted-foreground"><LoaderCircle className="mb-2 animate-spin" />Loading skills…</div> : skills.length === 0 ? <div className="space-y-3"><EmptyState icon={Wrench} title="No skills available" description="Create a skill to give ARES a reusable, explicit workflow." /><div className="flex justify-center"><Button asChild><Link to="/control?tab=skills&view=studio"><Plus />Create skill</Link></Button></div></div> : <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{skills.map((skill) => <Card key={skill.name}><CardContent className="space-y-3 pt-5"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="truncate font-medium">{skill.name}</div><Badge variant="outline" className="mt-1">{skill.category || "uncategorized"}</Badge></div><ToggleSwitch checked={!skill.disabled} disabled={working === skill.name} onCheckedChange={(enabled) => void toggle(skill, enabled)} /></div><p className="min-h-10 text-sm text-muted-foreground">{skill.description || "No description provided."}</p><div className="flex justify-end gap-2"><Button size="sm" variant="outline" asChild><Link to={`/control?tab=skills&view=studio&name=${encodeURIComponent(skill.name)}`}><Pencil />Edit</Link></Button><Button size="sm" variant="outline" disabled={working === skill.name} onClick={() => void remove(skill)} aria-label={`Delete ${skill.name}`}><Trash2 className="text-destructive" /></Button></div></CardContent></Card>)}</div>}
    </div>
  );
}
