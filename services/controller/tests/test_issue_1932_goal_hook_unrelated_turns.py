...
def test_start_chat_stream_accepts_goal_related():
    ...
    This was asserted against a source literal of the kwargs dict, which is what
    caught the regression where a third key was added to ``worker_kwargs`` that
    three of the four stream targets could not accept. The intent — goal_related
    actually reaches the worker — is now checked against the real signatures, so
    it survives adding a kwarg but still fails if goal_related stops arriving.
    """
    import inspect

    from api.chat_runtime import _filter_kwargs_for_callable
    ...
    for module_name, attr in targets:
        ...
        assert bound.get("goal_related") is True, (
            f"{attr} must receive goal_related from chat_runtime"
        )
        # And the call the runtime actually makes must bind without TypeError.
        inspect.signature(target).bind(
            "session-id", "prompt", "model", "/tmp/ws", "stream-id", [], **bound
        )
