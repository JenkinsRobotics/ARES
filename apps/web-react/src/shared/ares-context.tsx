...
export interface AresContextValue {
...
export function AresProvider({ children }: { children: ReactNode }) {
  ...
  const [streamToolSteps, setStreamToolSteps] = useState<ToolStepData[]>([]);
  ...
  const refresh = useCallback(async () => {
    ...
    const [health, settings, sessions, workspaces, backendSelection] = await Promise.allSettled([
      ...
      withTimeout(aresApi.getAresBackend(), 8_000, "backendSelection"),
    ]);
    ...
    setSnapshot((previous) => ({
      ...
      selectedBackend: backendSelection.status === "fulfilled"
        ? backendSelection.value.current || backendSelection.value.default || previous.selectedBackend
        : previous.selectedBackend,
  ...
  const handleStreamEvent = useCallback((event: ChatStreamEvent, sessionId: string) => {
    if (event.type === "text") setStreamText((value) => value + event.text);
      ...
      setStreamTools((items) => [...items.filter((item) => item !== event.label), event.label]);
      if (event.step) {
        const incoming = event.step;
        setStreamToolSteps((prev) => {
          const idx = prev.findIndex((s) => (incoming.id && s.id === incoming.id) || (!incoming.id && s.name === incoming.name && s.status === "running"));
          if (idx >= 0) {
            const copy = [...prev];
  ...
  const value = useMemo(() => ({
    ...
    streamToolSteps, streamStartTime,
  ...
  }), [snapshot, currentSession, selectedSessionId, streamText, streamReasoning, streamTools, streamToolSteps, streamStartTime, streamState, chatNotice, chatNoticeProvider, refresh, selectSession, createSession, sendMessage, cancelResponse, saveAssistantName]);
