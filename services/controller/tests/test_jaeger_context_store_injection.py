"""Context Store injection into JaegerAI ("jaeger") gateway turns.

Uses the same fake-HTTP-gateway technique as test_jaeger_backend_streaming.py's
...
class _FakeJrosGateway(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path.rstrip("/") == "/v1/health":
            body = json.dumps({"ok": True, "backend": "jaeger", "booted": True}).encode()
...
def _run_turn(monkeypatch, sid: str) -> dict:
    ...
    try:
        ...
        jaeger_gateway_chat.run_jaeger_streaming(
            sid, "hello jaeger", "test-model", "/tmp", stream_id, [],
...
def test_context_store_chunks_prepended_as_system_message(monkeypatch, tmp_path):
    ...
    body = _run_turn(monkeypatch, "jaeger-ctx-1")

    assert body["messages"][0]["role"] == "system"
    assert "FastAPI" in body["messages"][0]["content"]
    assert body["messages"][-1] == {"role": "user", "content": "hello jaeger"}
...
def test_no_injection_when_retrieval_returns_empty(monkeypatch):
    ...
    body = _run_turn(monkeypatch, "jaeger-ctx-2")

    assert body["messages"] == [{"role": "user", "content": "hello jaeger"}]
...
def test_no_injection_when_retrieval_raises(monkeypatch):
    ...
    body = _run_turn(monkeypatch, "jaeger-ctx-3")
...
def test_local_bridge_path_never_calls_context_store_retrieve(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(jaeger_gateway_chat, "jaeger_gateway_base_url", lambda cfg: "http://127.0.0.1:1")
    monkeypatch.setattr(jaeger_gateway_chat, "local_jaeger_root", lambda: tmp_path)
    monkeypatch.setattr(
        jaeger_gateway_chat, "_run_local_jaeger_turn",
        lambda msg_text, session_id, cancel_event, put_jaeger_event, stream_id: ("local reply", "", []),
    )

    sid = "jaeger-ctx-local"
    stream_id = f"stream-{sid}"
    session = Session(session_id=sid, messages=[])
    session.active_stream_id = stream_id
    session.pending_user_message = "hello jaeger"
