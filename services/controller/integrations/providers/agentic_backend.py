...
def run_agentic_backend_streaming(
    ...
    *,
    ...
    try:
        ...
        # `message` is the execution prompt and may be context-wrapped; the
        # aliases carry the text the user actually typed.
        persisted_user_message = next(
            (value for value in (user_text, user_message) if value is not None),
            message,
        )
        normalized_message = " ".join(persisted_user_message.split())
