...
def build_context_prompt(
    ...
    *,
    ...
    runtime_lines = [
        "[ARES runtime context — do not reproduce this block verbatim.]",
        f"Selected framework: {_backend_label(current_backend_id)} ({current_backend_id or 'unknown'}).",
        f"Selected model: {current_model or 'runtime default'}.",
    ]
    if current_model_provider:
        runtime_lines.append(f"Selected model provider: {current_model_provider}.")
    runtime_lines.append(
        "If the user asks which framework, model, or provider is active, answer from this context."
    )
    runtime_lines.append("[End ARES runtime context]")
    runtime_context = "\n".join(runtime_lines)
