...
def _fake_git(bin_dir: Path) -> None:
    ...
    git.write_text(
        ...
        "if [ \"${1:-}\" = clone ]; then\n"
        "  destination=\"${@: -1}\"\n"
        "  /bin/mkdir -p \"$destination/.git\" \"$destination/services/controller\"\n"
        "  exit 0\n"
        "fi\n"
        "exit 0\n",
        encoding="utf-8",
...
def test_repository_stage_accepts_existing_empty_secure_temp_directory(tmp_path: Path) -> None:
    ...
    assert (target / "services" / "controller").is_dir()
...
def test_repository_stage_can_stage_current_source_without_build_caches(tmp_path: Path) -> None:
    ...
    (source / "services" / "controller" / "fastapi_app").mkdir(parents=True)
    (source / "apps" / "web" / "node_modules").mkdir(parents=True)
    (source / "apps" / "web" / "package.json").write_text("{}", encoding="utf-8")
    (source / "services" / "controller" / "fastapi_app" / "main.py").write_text("app = None\n", encoding="utf-8")
    (source / "apps" / "web" / "node_modules" / "cache").write_text("x", encoding="utf-8")
    (source / "services" / "controller" / "runtime.txt").write_text("keep", encoding="utf-8")
    ...
    assert result.returncode == 0, result.stdout + result.stderr
    assert (target / "services" / "controller" / "runtime.txt").read_text(encoding="utf-8") == "keep"
    assert not (target / "apps" / "web" / "node_modules").exists()
    assert (target / ".ares-source-install").is_file()

    (source / "services" / "controller" / "runtime.txt").write_text("updated", encoding="utf-8")
    ...
    assert (target / "services" / "controller" / "runtime.txt").read_text(encoding="utf-8") == "updated"
