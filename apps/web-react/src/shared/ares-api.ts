...
export const aresApi = {
  ...
  async getDelegationLogs() {
    // Poll the local filesystem via a controller endpoint that reads ~/.hermes/cache/delegation/live/
    return apiFetch<{ delegations: Array<{ id: string; taskCount: number; logPaths: string[]; goals: string[]; status?: string }> }>("/api/delegation/logs");
  },
  async readDelegationLog(logPath: string) {
    // Read a specific log file content
    const payload = await apiFetch<{ content: string }>(`/api/delegation/log?path=${encodeURIComponent(logPath)}`);
    return payload.content || "";
  },
  ...
  async getAresBackend(sessionId?: string) {
    const query = sessionId ? `?session_id=${encodeURIComponent(sessionId)}` : "";
    return apiFetch<{ current: string; default: string; scope: string }>("/api/ares/backend" + query);
  },
...
export interface RAGSourcesConfig {
  sources: RAGSource[];
  enabled: boolean;
...
export interface RAGScanResult {
  path: string;
  status: "scanned" | "error" | "not_found";
  files?: number;
  index?: string[];
  error?: string;
}
...
// ── RAG Sources API ───────────────────────────────────────────────────
export async function getRAGSources(): Promise<RAGSourcesConfig> {
  return apiFetch("/api/ares/rag-sources");
}

export async function setRAGSources(config: Partial<RAGSourcesConfig>): Promise<{ ok: boolean; count: number }> {
  return apiFetch("/api/ares/rag-sources/set", {
    method: "POST",
...
export async function scanRAGSources(): Promise<RAGScanResponse> {
  return apiFetch("/api/ares/rag-sources/scan", {
    method: "POST",
  });
}
