...
///
/// Foundation `Process.terminate()` only signals the direct child. Uvicorn
/// then the Jaeger bridge then ARESNativeMCP sit under that child, and a
/// SIGKILL (or Force Quit of the app) reparents them to launchd. Snapshot
...
public enum ProcessTree {
    ...
    public static func descendants(of root: pid_t) -> [pid_t] {
    ...
    public static func terminate(root: pid_t, graceSeconds: TimeInterval = 4) async {
        guard root > 1 else { return }
    ...
    /// PIDs listening on `port` (TCP LISTEN). Empty when nothing is bound.
    public static func listeningPids(port: Int) -> [pid_t] {
        ...
        return output.split { $0.isNewline || $0.isWhitespace }
            .compactMap { pid_t($0) }
            .filter { $0 > 1 }
    }
    ...
    public static func startParentDeathWatchdog(parent: pid_t, root: pid_t) -> Process? {
    ...
    public static func parentDeathWatchdogScript(parent: pid_t, root: pid_t) -> String {
        """
        parent=\(parent)
        root=\(root)
        collect() {
