...
Desktop Companion is a trusted local companion extension for ARES. It
...
This entry is the first sidecar-class extension candidate for the ARES
...
not mean ARES core renders a pet inside the page.
...
In ARES, open Settings -> Extensions -> Gallery and install
Desktop Companion.
...
Start ARES with this extension directory:
...
To disable the WebUI extension, restart ARES without:
...
This is trusted local code. The injected adapter runs in the ARES
...
- reads guarded ARES browser globals for live session state while no
...
The sidecar metadata is descriptive. It does not imply that ARES core
...
- ARES core's consent-gated sidecar proxy is shipped, but this external
