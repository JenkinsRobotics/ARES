...
def test_antigravity_does_not_advertise_an_invented_model():
    adapter = GeminiAntigravityAdapter(turn_starter=lambda *_args, **_kwargs: {})

    assert adapter.get_models(profile="default") == []
...
def test_connection_model_and_mcp_routes_use_registry(tmp_path: Path, monkeypatch):
    ...
    with TestClient(app, client=("127.0.0.1", 50000)) as client:
        ...
        backend_state = client.get("/api/ares/backend")
    ...
    assert backend_state.status_code == 200
    assert backend_state.json()["status"] == {"recording": True, "offline": False}
