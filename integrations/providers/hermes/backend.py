...
Routes interactive chat through the separately managed Hermes Gateway when
that connection is configured. The Hermes CLI remains a compatibility fallback
for installations without a gateway connection.
...
# ARES connection ids / aliases -> Hermes provider slugs (WebUI session.model_provider).
_ARES_TO_HERMES_PROVIDER = {
    "xai_cloud": "xai-oauth",
    "grok_local": "xai-oauth",
    "codex_local": "openai-codex",
    "openai_cloud": "openai-codex",
    "ollama_cloud": "ollama-cloud",
}

_KEEP_HERMES_PROVIDERS = {"xai-oauth", "openai-codex", "ollama-cloud"}


def resolve_hermes_route(model: str | None, model_provider: str | None) -> tuple[str, str]:
    """Map an ARES turn (model + provider chip) to Hermes CLI -m/--provider.

    Hermes WebUI does this in resolve_model_provider(): never pass an ARES
    backend id as a Hermes provider, and infer provider from the model id
    when the UI only sent hermes_local.
    """
    default_model, default_provider = resolve_hermes_defaults()
    model = (model or "").strip() or default_model
    provider = (model_provider or "").strip()

    if provider in _ARES_TO_HERMES_PROVIDER:
        provider = _ARES_TO_HERMES_PROVIDER[provider]
    elif provider in ARES_BACKEND_IDS:
        provider = ""

    if not provider:
        low = model.lower()
        # Ollama Cloud tags (gemma4:31b, gpt-oss:20b) before gpt* -> Codex.
        if ":" in model or low.startswith(("gemma", "llama", "qwen", "deepseek", "glm", "mistral", "phi", "nemotron", "kimi", "minimax")):
            provider = "ollama-cloud"
        elif low.startswith("grok") or "/grok" in low:
            provider = "xai-oauth"
        elif low.startswith("gpt") or "codex" in low:
            provider = "openai-codex"
        else:
            provider = default_provider or "xai-oauth"
...
class HermesBackend(AgenticBackend):
    """Hermes Agent adapter with Gateway-first interactive chat."""
    ...
    def get_worker_target(self) -> tuple:
        """Return the configured Hermes transport without changing ARES identity.

        ARES and Hermes WebUI are peer clients of Hermes Gateway. ARES keeps
        ownership of its product session while the gateway owns Hermes runtime
        execution and native continuation.
        """
        from api.gateway_chat import hermes_gateway_connection_configured

        if hermes_gateway_connection_configured():
    ...
    def run_turn(self, message: str, session_id: str, **kwargs) -> Dict[str, Any]:
        ...
        model, provider = resolve_hermes_route(
            _cfg_str(config, "model") or str(kwargs.get("model") or "").strip(),
            _cfg_str(config, "provider") or str(kwargs.get("model_provider") or "").strip(),
        )
    ...
    def inventory(self) -> Dict[str, Any]:
        ...
        from api.gateway_chat import (
            _gateway_base_url,
            hermes_gateway_connection_configured,
        )

        gateway_configured = hermes_gateway_connection_configured()
        ...
        transports = [
            transport_entry(
                ...
                in_use=not gateway_configured,
                endpoint=cli or "hermes",
                notes="Compatibility fallback when no authenticated Hermes Gateway is configured.",
            ),
            transport_entry(
                id="mcp_serve",
                kind="mcp",
        ...
        gateways = [
            gateway_entry(
                ...
                endpoint=gateway_url or "not configured",
                in_use=gateway_configured,
                protocol="openai-compatible-sse",
                notes="ARES and Hermes WebUI connect to this gateway as peer clients.",
            ),
        ]
        ...
        return finalize_inventory(
            {
                ...
                "active_execution": {
                    "available": available,
                    "version": version,
                    "transport": "hermes_gateway" if gateway_configured else "cli_chat",
                    "model": active_model,
